...
<main class="base-page">
  ...
  <section class="posts">
    {#each data.posts! as post (post.id)}
      <a class="raw" href={resolve("/bedroom/posts")}>
        <main>
          <span>
            <b>{post.title}</b>
  ...
  {#if data.query}
    <section class="pagination">
      <form method="get">
        <Button size="icon"><ChevronsLeft /></Button>
        ...
        <label for="p">
          <input
            ...
            name="p"
            value={data.query.page}
            onblur={(e) => {
              if (e.currentTarget.valueAsNumber === data.query.page) return;
        ...
        <Button onclick={(e) => {}} size="icon">
          <ChevronRight />
        </Button>
        <Button size="icon"><ChevronsRight /></Button>
  ...
  <form method="post" action="?/thing" use:enhance>
    <Button type="submit" size="tiny">add thing</Button>
  </form>
</main>
